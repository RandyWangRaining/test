"""
================================================================================
FedTMC + Post-hoc Temperature Scaling (Post-hoc TS)

Core question:
  "If FedTMC+TS as a post-hoc calibration method can be a low-cost competitor?"

Experiment Design:
  1. Split 20% from the training set as a validation set (used for temperature scaling to avoid information leakage)
  2. Train standard FedTMC (DS fusion, no calibration module) using the remaining 80% training set
...
class TMCServer:
    ...
    def fuse_and_classify(self, evidences, labels):
        alpha_dict = {v: self.evidence_to_alpha(evidences[v]) for v in sorted(evidences.keys())}
        ...
        for i in range(1, len(alpha_list)):
            alpha_fused = self.ds_combine_two(alpha_fused, alpha_list[i])
    ...
    def predict(self, evidences):
        with torch.no_grad():
            alpha_list = [self.evidence_to_alpha(evidences[v]) for v in sorted(evidences.keys())]
            alpha_fused = alpha_list[0]
            for i in range(1, len(alpha_list)):
                alpha_fused = self.ds_combine_two(alpha_fused, alpha_list[i])
            probs = self.alpha_to_prob(alpha_fused)
        return probs, alpha_fused
...
def run_experiment(data_path, dataset_name='Scene15', epochs=300, runs=1,
    ...
    for run in range(runs):
        ...
        clients = []
        for v in range(dataset.num_views):
            client = TMCClient(v, dataset.dims[v], hidden_dim, dataset.num_classes,
                               lr=lr, device=device)
            clients.append(client)

        server = TMCServer(dataset.num_classes, dataset.num_views,
                           annealing_epochs=50, device=device)

        # ===== Train FedTMC =====
        print(f"\n--- Training FedTMC (DS fusion, no calibration) ---")
        ...
        print(f"\n  FedTMC (no TS):  Acc={metrics_no_ts['accuracy']*100:.2f}%  "
    ...
    # ===== Summary =====
    print(f"\n{'='*80}")
    print(f"SUMMARY: FedTMC vs FedTMC+TS on {dataset_name}")
    print(f"{'='*80}")
    ...
    print(f"{'FedTMC (no TS)':<30}{avg_no_ts['accuracy']*100:<15.2f}{avg_no_ts['ece']*100:<15.2f}{avg_no_ts['overconf']:<+15.4f}")
    print(f"{'FedTMC + TS (min-NLL)':<30}{avg_ts_nll['accuracy']*100:<15.2f}{avg_ts_nll['ece']*100:<15.2f}{avg_ts_nll['overconf']:<+15.4f}")
    print(f"{'FedTMC + TS (min-ECE)':<30}{avg_ts_ece['accuracy']*100:<15.2f}{avg_ts_ece['ece']*100:<15.2f}{avg_ts_ece['overconf']:<+15.4f}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='FedTMC + Post-hoc Temperature Scaling')
